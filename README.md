...
### v3.3.5 (2026-05-28)
- **隧道服务稳定性全面优化**
  - **启动超时优化**：超时时间从15秒增加到20秒，超时后返回成功状态而非失败
  - **心跳检测优化**：超时时间从10秒增加到15秒，失败阈值从5次增加到10次，心跳间隔从30秒增加到60秒
  - **重启计数器重置**：重启成功后自动重置tunnel_restart_count为0，避免显示136次重启的情况
  - **状态检测逻辑优化**：有有效URL时认为隧道在运行，增加详细日志输出
  - **URL验证超时优化**：从10秒增加到15秒，提高网络波动容忍度
  - **重启成功判断优化**：只要启动成功就重置计数器，即使URL还未完全就绪
  - **URL复用逻辑优化**：复用URL时同步old_tunnel_url，无效URL自动清理，重启时重置old_tunnel_url避免误判
  - **URL解析增强**：增加详细调试信息，跳过重复URL，只使用与旧URL不同的新URL
  - **进程清理优化**：Windows使用wmic精确匹配hostc进程，Linux/Mac使用pkill，确保跨系统兼容
  - **状态API优化**：始终优先从tunnel_url.txt读取最新URL，确保前端显示最新地址
  - **邮件通知优化**：URL变化时发送通知，复用URL时不发送通知，实现真正的"无感换新"
  - **TIME_WAIT连接优化**：心跳间隔增加到60秒，减少TIME_WAIT连接数量
  - **错误处理增强**：捕获并输出所有异常，便于问题排查
  - **跨系统兼容性**：使用Environment类统一处理Windows/Linux/Mac系统差异
...
- **隧道日志输出优化**
  ...
  - 优化日志实时性，便于问题排查和监控
  - 修复进程清理逻辑，Windows 上只清理 hostc 相关进程，避免误杀其他 node.js 程序
  - 改进隧道启动超时检测，15秒内未获取URL时返回失败状态
  - 确保失败计数器正确重置，避免无限重启循环

### v3.3.3 (2026-05-23)
- **隧道进程泄漏和邮件通知问题修复**
  - 修复隧道频繁重启导致258个node.exe进程同时运行的问题
  - 启动新隧道前自动清理所有旧的hostc/node进程（Windows: taskkill, Linux/Mac: pkill）
  - 重启隧道时自动清理旧进程，避免进程泄漏
  - 修复心跳检测过于敏感导致频繁误判重启的问题
  - 心跳检测超时从5秒增加到10秒，失败阈值从2次提高到5次
  - 添加详细的心跳检测异常日志输出，便于问题诊断
  - 修复邮件通知冷却时间过长导致部分URL变化不发邮件的问题
  - 邮件冷却时间从5分钟缩短到1分钟，失败冷却期从10分钟缩短到5分钟
  - 失败计数器在冷却期过后自动重置，避免永久禁用邮件通知
  - 增强SMTP错误处理，区分认证失败、连接失败等具体错误类型
  - 添加详细的邮件发送日志，追踪发送过程的每个步骤
  - 优化URL变化检测逻辑，确保每次URL变化都能正确触发邮件通知
  - 添加连续重启尝试的冷却机制，防止频繁重启（3次后启用冷却，最长5分钟）
  - 所有修改均支持跨平台（Windows/macOS/Linux），使用Environment.IS_WINDOWS判断系统类型
...
- **优化日志输出**
  ...
  - 添加启动失败检查，提前发现端口占用等问题
- **改进进程管理**
  - 使用 `wait` 命令等待子进程，窗口关闭时自动终止
  - Ctrl+C 或关闭窗口都能正确清理进程
...
- **隧道频繁重启和邮件发送问题修复**
  ...
  - 如果 URL 不可用，跳过复用逻辑，启动新的 hostc 进程
  - 添加失败退避机制：连续失败 5 次后增加等待时间（最多 60 秒）
  - 连续失败超过 15 次后自动停止重启，避免无限重试
  - 重启失败后自动清理 tunnel_url.txt 中的无效 URL
  - 成功启动隧道后重置连续失败计数器
  - 修复邮件发送 "Connection unexpectedly closed" 错误
  - 添加邮件发送冷却时间：两次发送间隔至少 5 分钟
  - 添加失败次数限制：连续失败 3 次后暂停发送 10 分钟
  - 避免因隧道频繁重启导致 SMTP 服务器拒绝连接

### v3.2.8 (2026-05-22)
- **Flask启动时邮件通知增强**
  - 修复复用已有隧道时不发送邮件通知的问题
  - 现在无论是新创建隧道还是复用已有隧道，都会发送邮件通知
  - 启动时自动检测并复用已有公网地址，同时发送邮件提醒
  - 确保用户在Flask启动时能收到公网地址邮件通知
  - 优化邮件通知触发时机，覆盖所有启动场景

### v3.2.7 (2026-05-22)
- **公网地址变更邮件通知**
  - 新增邮件通知功能，公网地址生成或变更时自动发送邮件
  - 发送目标：QQ 邮箱 (980187223@qq.com)
  - 新公网地址生成时发送「【新公网地址】」邮件
  - 重启后地址变更时发送「【更新公网地址】」邮件
  - 邮件包含：时间、公网地址、可点击链接
  - 支持纯文本和 HTML 两种格式
  - 新增 API 接口：`/api/email/config` (GET/POST)、`/api/email/test`
  - 配置项位于 `config.json`，包含 SMTP 服务器、端口、授权码等
...
- **隧道自动重连死循环修复**
  ...
  - 复用已有 URL 前先验证其可用性（发送 HEAD 请求测试）
  - 如果 URL 不可用，跳过复用逻辑，启动新的 hostc 进程
  - 启动新进程前清理可能存在的旧 hostc 进程，避免端口冲突
  - 重启成功后自动更新 tunnel_url.txt 文件，确保下次重启时使用最新 URL
  - 前端每 5 秒自动轮询隧道状态，URL 更新后无需刷新页面即可显示新地址
  - 实现真正的"无感切换"到新的公网 URL

### v3.2.1 (2026-05-20)
- **守护线程重启时保持 URL 一致**
  - restart_tunnel() 函数优先复用 tunnel_url.txt 中的已有 URL
  - 检测 hostc 进程状态，避免重复启动
  - 重启时不再生成新的公网地址，保持 URL 稳定
  - 解决前端页面和 tunnel_url.txt 显示不同 URL 的问题

### v3.2.0 (2026-05-20)
- **外部启动隧道监控机制**
  - 检测到外部启动的 hostc 隧道时，自动启动心跳和自动重启守护线程
  - 心跳检测不再依赖内部进程状态，只要有 tunnel_url 就发送心跳
  - 自动重启守护线程同时监控内部进程和外部 node.exe 进程
  - 重启时清空 tunnel_url，防止心跳继续检测已失效的旧 URL
  - 添加 tunnel_daemon_started 标志，避免重复启动守护线程
  - 前端无需刷新即可自动检测并显示外部隧道状态

### v3.1.9 (2026-05-20)
- **前端隧道共享按钮优化**
  - 点击"隧道共享"按钮时优先复用 `tunnel_url.txt` 中的已有地址
  - 不再重复启动 tunnel 进程，避免生成新的公网地址
  - 只有在文件不存在或无法读取时才启动新的 tunnel
  - 大幅提升按钮响应速度，直接使用 run.bat/run.sh 启动时生成的地址
...
- **隧道保持在线机制增强**
  - 心跳检测失败时也能触发自动重启（连续失败 2 次）
  - 移除最大重启次数限制，隧道可无限保持在线
  - 只要 Flask 服务器在运行，隧道就会一直保持连接
  - 重启机制同时监控进程状态和连接有效性

### v3.1.7 (2026-05-20)
- **货号对比重复检测优化**
  - 后端 `/api/sku/compare/txt` API 新增重复货号检测逻辑
  - 保存原始货号数据用于检测重复（不直接去重）
  - 调用 `find_duplicate_stock_numbers` 方法检测并返回重复信息
  - API 返回结果新增 `duplicate_count` 和 `duplicates` 字段
- **前端重复货号列表展示**
  - Excel 对比结果新增 **Excel重复货号列表**（橙色标签）
  - Excel 对比结果新增 **JSON重复货号列表**（粉色标签）
  - 货号对比功能新增 **重复货号列表**展示
  - 每个重复货号显示格式：`货号 (重复X次)`
- **标签描述修正**
  - "JSON中多余货号列表(高价商品≥599)" 改为 "JSON中多余货号(有货但不在输入中)"
  - 更准确描述该列表的实际含义
...
- **run.bat 服务启动后被意外终止修复**
  ...
  - 意外杀掉所有 Python 和 Node 进程的问题
  - 在 "启动完成" 后添加 `exit /b 0`，确保脚本正常退出而不执行清理代码
  - 添加更清晰的启动完成提示，包括访问地址和隧道地址
...
- **版本号自动同步**
  ...
  - 无需手动更新 HTML，前端与后端版本保持一致
- **隧道启动优化**
  - 修复首次点击"隧道共享"按钮时公网地址不显示的问题
  - 后端 `/api/tunnel/start` API 等待 URL 生成后再返回（最多15秒）
  - 前端启动隧道后持续轮询状态直到获取到 URL
  - 现在点击一次即可立即显示公网地址
- **启动脚本优化**
  - 修复 `run.bat` 和 `run.sh` 启动顺序问题
...
- **静态资源传输优化**
  ...
  - 减少隧道带宽占用，首屏加载速度提升 70-80%

### v3.1.1 (2026-05-18)
- **前端版本号同步**
  - 前端页面版本号现自动跟随 `main.py` 中的 `VERSION` 变量
  - 只需修改一处版本号，前端页面和 README 即可同步更新
- **前端代码优化**
  - 修复 `updateTunnelUI` 函数中元素可能为 null 的问题
  - 为所有 DOM 操作添加空值检查，提高稳定性
  - 添加调试日志，便于排查隧道状态问题

### v3.1.1 (2026-05-20)
- **隧道复制按钮修复**
  - 修复复制按钮点击后 URL 无法复制到剪贴板的问题
  - 修复 `innerHTML` 替换导致按钮事件绑定失效的 bug
  - 在 `data-url` 属性中直接设置 URL 值
  - 替换后重新获取按钮并绑定点击事件
  - 复制成功后按钮显示"已复制"提示

### v3.1.0 (2026-05-18)
- **隧道连接优化**
  - 修复 hostc 隧道无法连接本地服务的问题
  - 添加 `--local-host 0.0.0.0` 参数确保隧道正确连接到本地服务
  - 优化 URL 解析逻辑，过滤掉 `0.0.0.0` 和 `localhost` 等本地地址
- **复制功能修复**
  - 修复通过 hostc 隧道访问时复制按钮不工作的问题
  - 使用 `document.execCommand('copy')` 替代 `navigator.clipboard.writeText()`
  - 使用事件委托处理动态创建的按钮点击事件
  - 通过 `dataset.url` 方式设置 URL，避免字符串拼接转义问题
  - 添加空值检查，避免元素不存在时报错
  - 复制成功后按钮文字变为"已复制"，2秒后恢复
...
- **隧道共享功能增强**
  ...
  - 新增"复制"按钮，一键复制链接到剪贴板
  - 复制成功显示绿色提示，2秒后自动消失
  - 按钮颜色统一为蓝色，与其他功能按钮保持一致
  - 移除所有SSH相关错误提示和说明
- **启动脚本优化**
  - Windows (`run.bat`) 和 Linux/Mac (`run.sh`) 启动时预下载 hostc 服务
  - 加快首次点击"隧道共享"按钮时的启动速度

### v3.0.7 (2026-05-17)
- **隧道共享功能优化**
  - 公网地址轮询间隔从 1000ms 缩短至 500ms，加快获取速度
  - 最大重试次数从 15 次提升至 30 次，确保稳定获取 URL
  - 启动时立即返回已解析的 URL，而非占位符
  - 失败时显示更友好的提示信息
- **跨平台兼容性增强**
  - `subprocess.CREATE_NO_WINDOW` 使用 `getattr()` 安全检测，避免 Linux/Mac 上的 AttributeError
  - 统一使用 `Environment.IS_WINDOWS` 布尔值判断系统类型
  - 移除硬编码的 Windows 系统检查，改用更通用的方式
- **项目清理**
  - 删除 `hostc/` 源码目录（不影响 `npx hostc@latest` 远程下载）
  - 删除 `dist/hostc/` 构建目录

### v3.0.6 (2026-05-06)
- **天气时钟看板集成**
  - 新增独立的"天气时钟看板"区块，使用 iframe 嵌入 `dist` 应用
  - 看板位于功能特性下方，作为独立页面区块展示
  - 导航栏新增"天气看板"快速链接
  - 实现完整的响应式高度控制：超小屏(260px) → 小屏(280px) → 中屏(400px) → 大屏(480px) → 更大屏(520px) → 超大屏(560px)
- **页面结构优化**
  - 将天气时钟看板从首页 hero 区域独立出来
  - Hero 区域恢复为纯 Szwego 爬虫介绍页面
  - 移除旧的响应式媒体查询（weather-iframe-wrapper / weather-mobile-link）
  - 导航栏时间与 Cookie 状态显示恢复正常布局
- **删除/新增商品数据来源优化**
  - 今天运行过爬虫：从 JSON 的"小计"读取当天对比数据
  - 今天未运行爬虫：从 diff_log 读取昨天 vs 今天对比数据
  - 确保 API 返回的数据与爬虫运行结果一致
- **删除商品列表交互优化**
  - 移除删除商品列表的点击事件，避免无效弹窗
  - 删除的商品无法获取详情，不再触发点击提示
  - 提升用户体验，避免不必要的交互反馈
- **JSON文件对比排序修复**
  - 修复JSON文件排序逻辑，使用修改时间而非字母顺序排序
  - 确保正确获取上一个版本文件进行对比
  - 修复删除商品判定错误的问题
- **Excel与JSON对比功能修复**
  - 修复"新增高价商品"判定逻辑错误问题
  - 排除`_cache.json`缓存文件作为对比基准，改为对比正式JSON文件
  - 新增高价商品现在正确判定：必须既不在上一个JSON中、也不在Excel中
  - 修复26011、50613、79312、86615等货号被错误标记为"新增"的问题
...
- **移动端导航栏固定置顶优化**
  ...
  - 统一所有响应式断点的导航栏样式
- **移动端滚动体验优化**
  - 移除导致视口问题的`html, body { height: 100%; }`规则
  - 禁用移动端sticky定位，避免滚动异常
  - 确保页面内容可以正常滚动到底部
- **用户体验提升**
  - 用户滚动到任何位置都能看到导航栏
  - 可以随时点击导航栏链接快速跳转
...
### v3.0.4 (2026-05-01)
- **Excel文件路径去重优化**
  - 使用`dict.fromkeys()`对Excel文件路径进行去重
  - 转换为绝对路径后去重，确保同一文件不会被重复读取
  - 优化命令行模式和API模式的文件处理逻辑
- **货号读取顺序优化**
  - 保持Excel中货号的原始顺序，不再进行排序
  - 确保货号对比结果与Excel中的排列顺序一致
  - 使用列表推导式优化货号提取代码

### v3.0.2 (2026-05-01)
- **移动端响应式适配全面优化**
  - 修复移动端输出面板覆盖整个页面的问题，改为相对定位
  - 修复移动端统计信息（预计售出总价、平均售出均价、平台手续费）显示问题
  - 优化统计区域布局，改为横向排列，两列布局
  - 修复移动端表格高度不一致问题，统一最小高度
  - 修复移动端无法滚动的问题，优化滚动体验
  - 修复移动端内容被截断的问题，添加文本换行支持
  - 修复移动端底部按钮和页脚不可见的问题
  - 优化输出面板滚动逻辑，移动端不自动滚动到底部
  - 为爬虫执行结果和商品数据汇总卡片添加特定样式
...
- **Excel多文件读取优化**
  ...
  - 提供详细的读取日志，显示每个文件的读取情况
- **Excel对比功能增强**
  - `compare_excel_with_json()`方法现在会读取所有存在的Excel文件
  - Web API的`compare_sku_excel()`函数支持多文件读取
  - 在对比结果中显示使用的Excel文件列表和数量
- **配置文件优化**
  - 调整`excel_files`顺序，优先使用项目目录中的Excel文件
  - 支持多个Excel文件路径配置，自动查找存在的文件
- **问题修复**
  - 解决用户在不同位置修改Excel文件导致数据不一致的问题
  - 统一读取所有存在的Excel文件，避免遗漏数据
...
- **交互优化**
  - 点击绿色（已存在）或黄色（多余）的货号可查看商品详情
  - 灰色（缺失）的货号点击无反应，避免无效弹窗
  - 添加操作提示说明，提升用户体验
- **代码优化**
  - 重构货号对比API，提取公共函数`get_latest_json_products()`
  - 消除重复代码，提升代码可维护性
  - 优化JSON数据解析逻辑
