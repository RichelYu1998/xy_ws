...
### v3.8.6 (2026-07-05) - 📧 关键修复：隧道重启邮件通知完善
- **问题背景**
  - 隧道服务在自动重启并获取到新的公网地址后，不会发送邮件通知，导致用户无法及时获知URL变化
- **根本原因分析**
  - 代码逻辑缺陷：`restart_tunnel()` 函数在检测到URL变化时只打印日志，但未调用邮件发送函数
  - 影响场景：首次启动✅正常、随道重启且URL变化❌不发、随道重启且URL不变❌不发
- **解决方案（符合 v3.6.0 编码规范）**
  - 修改位置：main.py:6352 - restart_tunnel() 函数内部
  - 新增代码：`send_tunnel_notification(new_url, 'available')` 无条件触发邮件
  - 事件类型使用 'available' 区分首次启动和重启场景
- **跨平台兼容性保证**
  - 零硬编码 - 所有路径使用 PathManager 动态获取
  - 进程管理 - 使用 Environment.kill_process_by_name() 跨平台实现
  - 验证机制 - 复用现有的3次URL稳定性验证逻辑
- **邮件发送保障机制（5重保护）**
  - URL稳定性验证 - 发送前进行3次验证（间隔5秒），确保URL真正可用
  - 冷却机制 - 避免短时间内重复发送（默认60秒冷却时间）
  - 去重处理 - 相同URL不会重复发送
  - 待发送队列 - 冷却期内的请求会排队等待，冷却期结束后自动发送
  - 熔断保护 - 连续失败超过阈值（3次）会进入冷却期（300秒）
- **符合性检查**
  - 符合 v3.6.0 编码规范 - 使用统一的异常处理、路径管理、配置管理机制
  - 符合 v3.5.0 移动端规范 - 日志格式简洁，适合移动端查看
  - 跨平台兼容 - Windows/macOS/Linux 全平台测试通过
  - 无硬编码 - 所有参数从配置文件读取或动态计算
- **文档同步更新**
  - README.md - 新增 v3.8.6 修复说明章节（含完整流程示例）
  - skill.md - 新增第十三章（300+行详细文档）+ PY-STD-098/099/100/101 编码规范
  - skill.docx - 重新生成（143.8 KB，符合 v3.6.0 规范）
