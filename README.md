...
**1. 修复 `pollOutput is not defined` 错误** 🔴
- ✅ **位置**: [dist/app.js:2904](file:///D:/ws/xy_ws/dist/app.js#L2904)
- ✅ **问题**: 代码中意外插入孤立的 `pollOutput` 文本导致 JavaScript 运行时错误
- ✅ **解决**: 删除错误的文本，恢复正常执行

**2. 统一爬虫结果卡片显示格式** ⭐⭐
- ✅ **问题**: 爬虫输出包含"对比文件:"时，被错误识别为"商品对比结果"，显示两个卡片
- ✅ **解决**: 
  - 合并所有卡片生成逻辑为统一的"🐛 爬虫执行结果"格式
  - 将类型判断从 `skuData.type = 'product'` 改为 `skuData.type = 'spider'`
  - 统一显示：总商品数、高价商品(≥599)、预计售出总价、平均售出均价、平台手续费
- ✅ **效果**: 只显示一个卡片，包含所有统计信息

**3. 优化数据解析逻辑** ⭐
- ✅ **增强价格解析**:
  - 支持多种格式：`¥184,420.00`、`184,420.00`、`184420`
  - 添加详细日志输出，便于调试
  - 解析失败时给出警告而非静默失败
- ✅ **智能字段映射**:
  - "新增高价商品数" → 同时设置 `skuData.newHighPrice` 和 `skuData.highPriceCount`
  - "对比文件:" → 提取旧文件和新文件路径
  - "新增商品数:" → 尝试提取总商品数
...
# 浏览器控制台日志
[对比卡片] ✓ 检测到对比文件，统一为爬虫类型
[对比卡片] ✓ 新增高价商品数: X
[对比卡片] ✓ 预计售出总价: ¥184,420.00
[对比卡片] ✅ 爬虫结果卡片已成功插入DOM
```

#### 📁 修改文件清单

- [dist/app.js](file:///D:/ws/xy_ws/dist/app.js) - 卡片格式统一 + 数据解析优化 + 默认值修改
- [README.md](file:///D:/ws/xy_ws/README.md) - 版本号更新 + 本版本日志

#### 🎯 用户影响

- ✅ **修复前**: 显示两个卡片（商品对比结果 + 爬虫执行结果），数值显示0和-
- ✅ **修复后**: 只显示一个统一的爬虫执行结果卡片，真实数值正确显示
- ✅ **兼容性**: 支持所有爬虫输出格式（纯爬虫、带对比、混合模式）
...
#### 🎯 入库相对时间实时计算
- **问题**: 商品详情弹窗中"入库时间"显示的相对时间（如"11分钟前"）是爬取数据时源API返回的静态字符串，不会随查看时间更新，导致时间显示过时
- **解决方案**: 
  - 基于`入库时间戳`（绝对时间）实时计算相对时间，每次打开商品详情时动态算出"X分钟前/X小时前/X天前"
  - 合并为一行显示：`🕐 入库时间: X分钟前（2026-07-26 18:06:05）`，同时展示相对时间和绝对时间
  - 移除过时的`入库时间`字段显示（源API返回的静态相对时间不再使用）
  - 颜色逻辑改为基于实际时间差计算，不再依赖文本匹配

#### 📝 技术细节
- **前端实现** ([index.html:2253-2288](file:///D:/ws/xy_ws/index.html#L2253-L2288)):
  - 用`new Date(p.入库时间戳)`获取绝对时间
  - 计算`now - createdDate`得到毫秒级时间差
  - 根据时间差动态生成相对时间字符串（刚刚/X分钟前/X小时前/X天前/X月前/X年前）
  - 颜色判断改为基于`diffHours`（实际时间差），不再匹配文本关键词
  - 合并显示：`${relativeTime}（${p.入库时间戳}）`
- **相对时间计算规则**:
  - < 1分钟 → "刚刚"
  - < 60分钟 → "X分钟前"
  - < 24小时 → "X小时前"
...
#### 🎨 用户体验提升
- ✅ 入库相对时间始终反映当前查看时刻，不再过时
- ✅ 一行同时展示相对时间和绝对时间，信息完整
- ✅ 颜色标识基于实际时间差，更准确可靠
- ✅ 无需重新爬取数据即可看到正确的相对时间

#### 📋 修改文件
- [index.html:2253-2288](file:///D:/ws/xy_ws/index.html#L2253-L2288) - 商品详情入库时间实时计算
...
#### 🎯 搜索功能多表联动
- **问题**: 搜索商品时只过滤"总商品列表"，"高价商品"等其他表格不受影响，两个表没有联动
- **解决方案**: 
  - 搜索时同时过滤所有4个表格（总商品、高价商品、高价新增、新增商品）
  - 每个表格标题显示各自的匹配数（如 `总商品列表 (89个) → 匹配 1/89 个`）
  - 无匹配的表格自动隐藏，标题显示 `→ 无匹配`
  - 清除搜索时恢复所有表格显示和标题

#### 📊 分表独立统计
- **问题**: 顶部统计区（预计售出总价、平均售出均价、平台手续费）显示所有表合并数据，不精确
- **解决方案**:
  - 每个表格标题下方添加独立的统计行（售出总价、均价、手续费）
  - 搜索时每个表各自计算和更新自己的统计数据
  - 顶部统计区只反映总商品列表的数据
  - 清除搜索时每个表的统计行恢复为原始数据

#### 🏷️ 顶部徽章联动更新
- 搜索时顶部徽章（总商品: 89个、高价(≥599): 75个）更新为匹配数（如 `总商品: 1/89个`）
- 无匹配的徽章半透明显示

#### 🏷️ 搜索结果分表展示
- 搜索结果区域按表分别展示匹配数，每个表单独一个彩色标签
- 如搜索"9000"时显示：`[总商品: 1/89]  [高价商品: 1/75]`
- 不再将所有表匹配数合并为"共找到 X 个匹配"

#### 📝 技术细节
- **前端实现** ([index.html:3196-3382](file:///D:/ws/xy_ws/index.html#L3196-L3382)):
  - 重构 `filterProducts()` 函数，遍历4个表格统一过滤
  - 新增 `renderTable()` 中每个表格的独立统计行（`tableId + '-stats'`）
  - 统计行使用 `data-original-html` 属性保存原始内容，清除搜索时恢复
  - 顶部徽章添加 `id` 和 `data-original-text`，搜索时动态更新
  - 搜索结果区域改为 `flex` 布局，每个表单独一个彩色标签
- **表格ID映射**:
  - `table-all` → 总商品列表 → `badge-total`
  - `table-highprice` → 高价商品 → `badge-highprice`
  - `table-highprice-new` → 高价新增 → 无徽章
  - `table-added` → 新增商品 → `badge-added`

#### 🎨 用户体验提升
- ✅ 搜索时所有表格联动过滤，不再只过滤总商品列表
- ✅ 每个表格独立显示自己的统计数据，不合并
- ✅ 顶部徽章实时更新匹配数
- ✅ 搜索结果分表展示，清晰直观
- ✅ 清除搜索后完全恢复原始状态
...
#### 📋 修改文件
- [index.html:3196-3382](file:///D:/ws/xy_ws/index.html#L3196-L3382) - filterProducts 多表联动 + 分表统计
- [index.html:3425-3456](file:///D:/ws/xy_ws/index.html#L3425-L3456) - renderTable 添加独立统计行
- [index.html:3448-3463](file:///D:/ws/xy_ws/index.html#L3448-L3463) - 顶部徽章添加id和data属性
- [index.html:3481-3483](file:///D:/ws/xy_ws/index.html#L3481-L3483) - 搜索结果区域改为flex布局
...
### v3.8.85 (2026-07-26) - 🔍 商品搜索统计优化

#### 🎯 搜索功能统计实时计算
- **问题**: 搜索商品时，统计数据（预计售出总价、平均售出均价、平台手续费）显示全部商品的统计，不够精确
- **解决方案**: 
  - 搜索时实时计算匹配商品的统计数据
  - 只搜索"总商品列表"表格，不影响其他表格显示
  - 清除搜索时恢复显示全部商品的统计数据

#### 📝 技术细节
- **前端实现** ([index.html:3196-3293](file:///D:/ws/xy_ws/index.html#L3196-L3293)):
  - 新增 `updateStatistics()` 函数，动态更新统计显示
  - 增强 `filterProducts()` 函数，实时计算搜索结果统计
  - 只搜索 `#table-all` 表格（总商品列表）
  - 其他表格（高价商品、高价新增、新增商品）保持不变
- **统计计算逻辑**:
  - 预计售出总价：匹配商品的售价总和
  - 平均售出均价：总售价 / 匹配商品数量
  - 平台手续费：总售价 × 1.6%
...
#### 📋 修改文件
- [index.html:3196-3293](file:///D:/ws/xy_ws/index.html#L3196-L3293) - 搜索统计实时计算
...
- **解决方案**: 
  - 移除商品列表表格中的"入库时间"列，保持列表简洁
  - 在商品详情页同时显示"入库时间戳"和"入库时间"两个字段
  - 两个字段都根据时间新鲜度显示不同颜色标识

#### 📝 技术细节
- **前端优化** ([index.html:2253-2280](file:///D:/ws/xy_ws/index.html#L2253-L2280)):
  - 将`else if`改为两个独立的`if`语句，同时显示两个字段
  - 第二个字段使用`+=`追加内容，避免覆盖
  - 移除商品列表表格中的入库时间列和相关逻辑
- **显示逻辑**:
  - 🕐 入库时间戳：显示精确时间（如"2026-07-24 15:50:21"）
  - 🕐 入库时间：显示相对时间（如"3小时前"）
  - 颜色标识：
    - 🟢 绿色：24小时内/刚刚/几分钟前
    - 🟠 橙色：24-72小时/几小时前/1天前
...
#### 📋 修改文件
- [index.html:2253-2280](file:///D:/ws/xy_ws/index.html#L2253-L2280) - 商品详情页同时显示两个字段
- [index.html:3293](file:///D:/ws/xy_ws/index.html#L3293) - 移除商品列表表格入库时间列
- [index.html:3307-3323](file:///D:/ws/xy_ws/index.html#L3307-L3323) - 移除入库时间相关逻辑

#### ✅ 验证结果
```
✅ 商品列表表格只显示：序号、货号、商品描述、售价、员工
✅ 商品详情页同时显示：入库时间戳、入库时间
✅ 两个字段都正确显示颜色标识
✅ 按钮功能正常，无JavaScript错误
```

---
### v3.8.81 (2026-07-24) - 🕐 入库时间数据源修复

#### 🎯 入库时间显示逻辑重构
- **问题**: 入库时间显示"0分钟前"，实际应显示商品在微购相册的发布时间
- **解决方案**: 
  - 从API响应的`time_stamp`字段获取精确时间戳（毫秒级）
  - 从API响应的`old_time`字段获取相对时间（如"2月前"）
  - 移除前端传递时间戳的错误逻辑
  - 优先显示精确时间，备用显示相对时间

#### 📝 技术细节
- **后端实现** ([main.py:4267-4294](file:///D:/ws/xy_ws/main.py#L4267-L4294)):
  - 新增 `入库时间戳` 字段：从API响应的`time_stamp`字段获取（毫秒级时间戳）
  - 新增 `入库时间` 字段：从API响应的`old_time`字段获取（相对时间）
  - 将时间戳转换为可读格式（如"2026-04-23 10:30:00"）
- **API优化** ([main.py:6719-6761](file:///D:/ws/xy_ws/main.py#L6719-L6761)):
  - 移除错误的`t`参数时间戳逻辑
  - 从商品列表中提取所有入库时间戳
  - 返回最早的入库时间（最新商品）
- **前端修复** ([index.html:2245-2297](file:///D:/ws/xy_ws/index.html#L2245-L2297)):
  - **商品详情弹窗展示每个商品自己的入库时间**
  - **商品列表表格添加"入库时间"列**
  - 优先显示精确时间戳（`p.入库时间戳`）
  - 备用显示相对时间（`p.入库时间`）
...
- ✅ 商品列表表格展示入库时间列
- ✅ 商品详情弹窗展示每个商品自己的入库时间
- ✅ 入库时间显示精确时间（如"2026-04-23 10:30:00"）
- ✅ 备用显示相对时间（如"2月前"、"1天前"）
- ✅ 不再显示错误的"0分钟前"
- ✅ 颜色标识准确反映数据新鲜度
- ✅ 需要重新爬取数据才能看到正确的入库时间
...
#### 📋 修改文件
- [main.py:4267-4294](file:///D:/ws/xy_ws/main.py#L4267-L4294) - 爬取数据保存精确时间戳
- [main.py:6719-6761](file:///D:/ws/xy_ws/main.py#L6719-L6761) - API返回入库时间逻辑
- [index.html:2245-2297](file:///D:/ws/xy_ws/index.html#L2245-L2297) - 商品详情弹窗展示逻辑优化
- [index.html:3284-3331](file:///D:/ws/xy_ws/index.html#L3284-L3331) - 商品列表表格添加入库时间列
- [skill.md:3205-3364](file:///D:/ws/xy_ws/skill.md#L3205-L3364) - 更新技术规范

#### ✅ 验证结果
```
✅ API返回的time_stamp字段正确转换为可读时间
✅ 商品列表表格展示入库时间列
✅ 商品详情弹窗展示每个商品自己的入库时间
✅ 入库时间显示精确时间（2026-04-23 10:30:00）
✅ 备用显示相对时间（2月前、1天前等）
✅ 颜色标识准确反映数据新鲜度
✅ 不再显示错误的"0分钟前"
```

### v3.8.80 (2026-07-24) - 🎨 商品详情入库时间颜色标识

#### 🎯 商品详情页入库时间展示优化
- **问题**: 商品详情页缺少入库时间信息，无法直观判断数据新鲜度
- **解决方案**: 
  - 在商品详情弹窗中添加入库时间字段
  - 根据入库时长设置不同颜色标识：
...
#### 📝 技术细节
- **前端实现** ([index.html:2249-2278](file:///D:/ws/xy_ws/index.html#L2249-L2278)):
  - 在showProductModal函数中添加入库时间展示
  - 从window.allProductsData获取全局入库时间
  - 根据created_time计算时间差
  - 动态设置颜色样式
- **颜色规则**:
  ```javascript
  if (hoursDiff <= 24) {
      colorStyle = 'color: #67c23a; font-weight: bold;'; // 绿色
  } else if (hoursDiff <= 72) {
      colorStyle = 'color: #E6A23C; font-weight: bold;'; // 黄色
  } else {
      colorStyle = 'color: #f56c6c; font-weight: bold;'; // 红色
  }
  ```
...
- ✅ 商品详情页直观展示入库时间
- ✅ 颜色标识快速判断数据新鲜度
- ✅ 绿色代表最新数据，黄色代表较新，红色代表较旧
- ✅ 提升用户决策效率

#### 📋 修改文件
- [index.html:2249-2278](file:///D:/ws/xy_ws/index.html#L2249-L2278) - 商品详情弹窗入库时间展示
...
### v3.8.79 (2026-07-24) - 📊 商品数据入库时间展示

#### 🎯 商品数据时间追踪功能
- **问题**: 商品数据缺少时间信息，无法判断数据新鲜度
- **解决方案**: 
...
#### 📝 技术细节
- **后端实现** ([main.py:6717-6747](file:///D:/ws/xy_ws/main.py#L6717-L6747)):
  - 新增 `created_time` 字段：从URL参数 `t` 解析时间戳
  - 新增 `storage_duration` 字段：智能计算入库时长
  - 时长格式化规则：
    - 超过1天：`X天X小时X分钟前`
    - 超过1小时：`X小时X分钟前`
    - 不足1小时：`X分钟前`
- **前端展示** ([index.html:3292](file:///D:/ws/xy_ws/index.html#L3292)):
  - 在商品数据汇总标题栏显示入库时间
  - 添加时钟图标增强视觉效果
  - 格式：`🕐 入库时间: X天X小时X分钟前`
...
#### 📋 修改文件
- [main.py:6717-6747](file:///D:/ws/xy_ws/main.py#L6717-L6747) - 时间计算逻辑
- [index.html:3292](file:///D:/ws/xy_ws/index.html#L3292) - 前端展示优化
...
#### 1. **高价商品统计显示为新增数量而非总数 - 最终修复**

**根本原因**: 
- 前端代码 `dist/app.js` 解析爬虫输出时，精确解析部分会覆盖预扫描结果
- 爬虫输出顺序: 先输出"售价 >= 599 的商品: X 个"(总数)，后输出"新增高价商品数: Y"(新增数)
- 精确解析匹配条件包含 `line.includes('高价商品') && line.includes(':')`，会错误匹配"新增高价商品数: Y"
- 结果: 正确的总数 X 被错误的新增数量 Y 覆盖

**修复方案**:
1. 预扫描部分: 排除包含"新增"关键词的行
2. 精确解析部分: 只匹配"售价 >= 599 的商品"，排除"新增高价商品"

**修复效果**:
- 高价商品数量正确显示所有高价商品总数(如73个)，而非新增数量(如1个)
...
#### 1. **高价商品(≥599)统计始终为0 - 彻底修复**

**根本原因**: 字段名不匹配
- 商品数据存储时使用字段名 '售价'（见 extract_product_info 方法）
- 但统计高价商品时代码查找字段名 'price'
- 导致所有商品的 price 字段都为空，高价商品数量始终为 0

**修复位置**: 共修改 13 处字段引用

| 行号 | 功能描述 |
|------|----------|
| 4496 | ilter_high_price_products 方法 - 筛选高价商品 |
| 4587 | 价格合并逻辑 |
| 4589 | 缓存价格获取 |
| 5079 | 解析商品价格 |
| 5209-5210 | 高价商品筛选条件 |
| 6668, 6710, 6845, 6892 | 商品价格获取 |
| 6991 | 调试输出 |
| 6998, 7312 | 统计高价商品 |
