...
- **自动差异日志**：对比结果自动保存到按日期命名的日志文件
- **追加模式**：同一天多次运行会追加记录，不会覆盖之前的数据
- **精确时间戳**：每次对比都记录精确的时间（格式：`YYYY-MM-DD HH:MM:SS`）
- **自动查找最新文件**：无需手动指定，自动找到最新的JSON文件
- **结构化日志**：JSON格式，便于后续处理和分析
- **菜单更新**：新增选项4用于Excel与JSON对比，原选项4（更新Cookie）调整为选项5

### v1.2.0 (2026-04-03)
- **Chrome浏览器支持**：使用系统已安装的Chrome浏览器，无需下载Chromium
- **每日独立文件**：每天生成独立JSON文件，文件名格式：`YYYYMMDD微购相册(小旭数码).json`
- **时间戳功能**：JSON文件添加精确时间戳，记录采集时间
- **数据对比小计**：自动对比上一个文件的变化，显示新增和删除的商品
- **JSON结构优化**：添加"生成日期"、"时间戳"、"小计"等字段
