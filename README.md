...
### v2.0.3 (2026-04-29)
- **新增商品列表联动滚动功能**：总商品列表与高价商品(≥599元)表格实现同步滚动
- **智能货号匹配**：当两个表格中存在相同货号的商品时，滚动一个表格会带动另一个表格滚动到相同货号位置
- **仅同步共有数据**：如果某个货号只在一个表格中存在（如普通价商品不在高价商品表中），则不会触发同步滚动
- **提升浏览体验**：方便用户在两个表格之间对比同一商品的展示位置
