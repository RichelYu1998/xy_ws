...
#### 更新内容: 修复移动端总商品列表与高价商品表格滚动联动抖动问题，滚动同步改为比例同步（不同行数表格滚动到底时都能到达实际最后一行），新增点击行联动高亮功能
...
- **滚动同步改为比例同步 (Bug修复)** — 滚动到底时另一个表格没到实际最后一行
  - 根因: 直接复制`scrollTop`绝对值，但两个表格内容高度不同（62行 vs 49行），同样的`scrollTop`值对应不同的相对位置
