...
function localOriginAdapter(options) {
  ...
  return {
    ...
    async handleWebSocket(request) {
      ...
      const socket = new import_ws.default(url, request.protocols, {
...
var ClientCreditController = class {
  ...
  async waitForOutbound(streamId, channelId, kind, bytes, canWait) {
    ...
    while (!this.hasOutbound(streamId, channelId, kind, bytes)) {
      await new Promise((resolve2) => this.waiters.add(resolve2));
      if (!canWait()) {
        throw new Error("stream unavailable");
      }
    }
...
function openWebSocket(url, token) {
  return new Promise((resolve2, reject) => {
    const socket = new import_ws3.default(url, {
...
async function waitForSocketCapacity(socket) {
  ...
  while (socket.readyState === import_ws3.default.OPEN && socket.bufferedAmount > DATA_SOCKET_BACKPRESSURE_LOW_WATERMARK) {
...
var ClientConnection = class {
  ...
  constructor(options) {
    ...
    this.disconnected = new Promise((resolve2) => {
      this.resolveDisconnected = resolve2;
    });
  }
  async connect() {
  ...
  async sendDataPayload(stream, kind, payload, flags = FRAME_FLAG_NONE) {
    ...
    for (let offset = 0; offset < payload.byteLength || offset === 0; ) {
      ...
      if (!socket || socket.readyState !== import_ws2.default.OPEN) {
  ...
  canSendForStream(stream) {
    return !stream.aborted && this.streams.isCurrent(stream) && this.sockets.get(stream.channelId)?.readyState === import_ws2.default.OPEN;
  }
...
var DEFAULT_DATA_CHANNELS2 = 2;
var MAX_DATA_CHANNELS2 = 8;
var DEFAULT_MAX_WEBSOCKET_MESSAGE_BYTES2 = 1024 * 1024;
var DEFAULT_MAX_METADATA_BYTES2 = 64 * 1024;
var DEFAULT_CHANNEL_CREDIT_BYTES2 = 4 * DEFAULT_MAX_WEBSOCKET_MESSAGE_BYTES2;
var MAX_CREDIT_BYTES2 = Number.MAX_SAFE_INTEGER;
var FRAME_CODE_REQUEST_START2 = 16;
var FRAME_CODE_REQUEST_DATA2 = 17;
var FRAME_CODE_REQUEST_END2 = 18;
var FRAME_CODE_REQUEST_ABORT2 = 19;
var FRAME_CODE_RESPONSE_START2 = 32;
var FRAME_CODE_RESPONSE_DATA2 = 33;
var FRAME_CODE_RESPONSE_END2 = 34;
var FRAME_CODE_RESPONSE_ABORT2 = 35;
var FRAME_CODE_STREAM_CREDIT2 = 48;
var FRAME_CODE_CHANNEL_CREDIT2 = 49;
var FRAME_TYPE_REQUEST_START2 = "request.start";
var FRAME_TYPE_REQUEST_DATA2 = "request.data";
var FRAME_TYPE_REQUEST_END2 = "request.end";
var FRAME_TYPE_REQUEST_ABORT2 = "request.abort";
var FRAME_TYPE_RESPONSE_START2 = "response.start";
var FRAME_TYPE_RESPONSE_DATA2 = "response.data";
var FRAME_TYPE_RESPONSE_END2 = "response.end";
var FRAME_TYPE_RESPONSE_ABORT2 = "response.abort";
var FRAME_TYPE_STREAM_CREDIT2 = "stream.credit";
var FRAME_TYPE_CHANNEL_CREDIT2 = "channel.credit";
var MAX_HEADER_VALUE_BYTES2 = 16 * 1024;
var MAX_HEADERS_BYTES2 = 128 * 1024;
var MAX_UINT642 = (1n << 64n) - 1n;
var textEncoder2 = new TextEncoder();
var textDecoder2 = new TextDecoder("utf-8", { fatal: true });
var FRAME_TYPE_TO_CODE2 = /* @__PURE__ */ new Map([
  [FRAME_TYPE_REQUEST_START2, FRAME_CODE_REQUEST_START2],
  [FRAME_TYPE_REQUEST_DATA2, FRAME_CODE_REQUEST_DATA2],
  [FRAME_TYPE_REQUEST_END2, FRAME_CODE_REQUEST_END2],
  [FRAME_TYPE_REQUEST_ABORT2, FRAME_CODE_REQUEST_ABORT2],
  [FRAME_TYPE_RESPONSE_START2, FRAME_CODE_RESPONSE_START2],
  [FRAME_TYPE_RESPONSE_DATA2, FRAME_CODE_RESPONSE_DATA2],
  [FRAME_TYPE_RESPONSE_END2, FRAME_CODE_RESPONSE_END2],
  [FRAME_TYPE_RESPONSE_ABORT2, FRAME_CODE_RESPONSE_ABORT2],
  [FRAME_TYPE_STREAM_CREDIT2, FRAME_CODE_STREAM_CREDIT2],
  [FRAME_TYPE_CHANNEL_CREDIT2, FRAME_CODE_CHANNEL_CREDIT2]
]);
var FRAME_CODE_TO_TYPE2 = new Map([...FRAME_TYPE_TO_CODE2.entries()].map(([frameType, code]) => [
  code,
  frameType
]));

// src/config.ts
var DEFAULT_SERVER_URL = "https://hostc.dev";
var DEFAULT_LOCAL_HOST = "localhost";
var CONFIG_ENV = "HOSTC_CONFIG";
var SERVER_URL_ENV = "HOSTC_SERVER_URL";
var DEBUG_ENV = "HOSTC_DEBUG";
var DISABLE_UPDATE_CHECK_ENV = "HOSTC_DISABLE_UPDATE_CHECK";
function getConfigPath(env = process.env) {
  if (env[CONFIG_ENV]) {
    return (0, import_node_path.resolve)(env[CONFIG_ENV]);
  }
  const home = (0, import_node_os.homedir)();
  if (!home) {
    throw new Error("Home directory is unavailable; set HOSTC_CONFIG");
  }
  return (0, import_node_path.join)(home, ".hostc", "config.json");
}
...
async function setConfigValue(key, value, env = process.env) {
  const config = await readConfig(env);
  config[key] = parseConfigValue(key, value);
  await writeConfig(config, env);
  return config;
}
async function unsetConfigValue(key, env = process.env) {
  const config = await readConfig(env);
  delete config[key];
  await writeConfig(config, env);
  return config;
}
async function resolveConfig(input = {}, env = process.env) {
  const fileConfig = await readConfig(env);
  const serverUrl = normalizeServerUrl(
    input.serverUrl ?? env[SERVER_URL_ENV] ?? fileConfig.serverUrl ?? DEFAULT_SERVER_URL
  );
  ...
  const dataChannels = validateDataChannels(
    input.dataChannels ?? fileConfig.dataChannels ?? DEFAULT_DATA_CHANNELS2
  );
...
function validateLocalHost(value) {
...
function validateDataChannels(value) {
  if (!Number.isInteger(value) || value < 1 || value > MAX_DATA_CHANNELS2) {
    throw new Error(
      `data-channels must be an integer from 1 to ${MAX_DATA_CHANNELS2}`
    );
  }
  return value;
}
...
async function runDoctor(options) {
  ...
  if (options.port !== void 0) {
    ...
    checks.push({
      ...
      message: local.ok ? `${host}:${options.port} is accepting connections` : `${host}:${options.port} is not accepting connections yet`
    });
  }
  return checks;
}
function printDoctorChecks(checks) {
  console.log(import_chalk.default.bold("Doctor"));
  for (const check of checks) {
    const label = check.status === "ok" ? import_chalk.default.green("OK") : check.status === "warn" ? import_chalk.default.yellow("WARN") : import_chalk.default.red("FAIL");
    console.log(`  ${label}  ${check.name}: ${check.message}`);
  }
}
function hasDoctorFailure(checks) {
  return checks.some((check) => check.status === "fail");
}
async function checkLocalPort(host, port, timeoutMs = 700) {
...
async function warnIfLocalPortClosed(host, port) {
  const result = await checkLocalPort(host, port);
  if (result.ok) {
    return;
  }
  console.error(
    `${import_chalk.default.yellow("Warning")}  Local service ${host}:${port} is not accepting connections yet.`
  );
  console.error(import_chalk.default.dim("         The tunnel will still start."));
}
async function checkServerHealth(serverUrl) {
  ...
  try {
    ...
    return {
      ...
      message: response.ok ? `${serverUrl} is healthy` : `${serverUrl} returned HTTP ${response.status}`
    };
  } catch (error) {
    return {
      name: "Server",
...
function localConnectHosts(host) {
...
var TerminalSpinner = class {
  ...
  render() {
    ...
    this.frameIndex += 1;
    this.stream.write(`\r\x1B[2K${import_chalk2.default.cyan(frame)} ${import_chalk2.default.dim(this.text)}`);
  }
};

// src/update.ts
var import_chalk3 = __toESM(require("chalk"));
function maybePrintUpdateNotice(currentVersion, options = {}) {
  if (options.disabled || !process.stderr.isTTY) {
    return;
  }
  void checkLatestVersion(currentVersion, options.fetcher ?? fetch).then((latest) => {
    if (!latest) {
      return;
    }
    console.error(
      `${import_chalk3.default.cyan("Update")}   hostc ${currentVersion} -> ${latest}`
    );
    console.error(import_chalk3.default.dim("         npm i -g hostc@latest"));
  }).catch(() => void 0);
}
async function checkLatestVersion(currentVersion, fetcher = fetch) {
  const controller = new AbortController();
  ...
  try {
    ...
    return body.version && compareVersions(body.version, currentVersion) > 0 ? body.version : null;
  } finally {
    clearTimeout(timeout);
  }
}
...
async function main() {
  const program = new import_commander.Command().name("hostc").description("Expose a local HTTP/WebSocket service through a hostc tunnel").version(CLI_VERSION).showHelpAfterError();
  program.argument("<port>", "local port to expose", parsePort).option(
    "--local-host <host>",
    "host of the local service",
    validateLocalHost
  ).option("--server <url>", "hostc server URL").option(
    "--data-channels <count>",
    "number of data channel WebSockets",
    parseDataChannels
  ).option("--qr", "print a terminal QR code for the public URL").action(async (port, options) => {
  ...
  config.command("get").description("Print config JSON").action(async () => {
    console.log(JSON.stringify(await readConfig(), null, 2));
  });
  config.command("set").argument("<key>", "server-url, local-host, data-channels, or qr").argument("<value>").description("Set a config value").action(async (key, value) => {
    const configKey = parseConfigKey(key);
    const next = await setConfigValue(configKey, value);
    console.log(JSON.stringify(next, null, 2));
  });
  config.command("unset").argument("<key>", "server-url, local-host, data-channels, or qr").description("Unset a config value").action(async (key) => {
    const configKey = parseConfigKey(key);
    const next = await unsetConfigValue(configKey);
    console.log(JSON.stringify(next, null, 2));
  });
  program.command("doctor").argument("[port]", "local port to check", parseOptionalPort).option("--server <url>", "hostc server URL").option(
    "--local-host <host>",
    "host of the local service",
    validateLocalHost
  ).description("Check hostc CLI, server, config, and optional local port").action(async (port, options) => {
...
async function runTunnel(port, options) {
  ...
  client.on("ready", (event) => {
    if (resolved.qr && process.stdout.isTTY && !printedFirstQr) {
      console.log((0, import_uqr.renderUnicode)(event.publicUrl));
      printedFirstQr = true;
    }
    const snapshot = client.getSnapshot();
    console.log(`${import_chalk4.default.green("Success")}  ${import_chalk4.default.bold("Tunnel ready")}`);
    console.log(`  Public URL: ${event.publicUrl}`);
    console.log(`  Local:      ${import_chalk4.default.dim(localOrigin.href)}`);
    console.log(`  Tunnel:     ${import_chalk4.default.dim(event.tunnelId)}`);
    console.log(`  Channels:   ${import_chalk4.default.dim(String(snapshot.dataChannels))}`);
  });
  client.on("reconnecting", (event) => {
    spinner.stop();
    console.error(
      `${import_chalk4.default.yellow("Reconnect")} ${formatError(event.reason)}; retrying in ${event.delayMs}ms`
    );
  });
  client.on("log", (event) => {
    if (resolved.debug) {
      const fields = event.fields ? ` ${formatError(JSON.stringify(event.fields))}` : "";
...
function parsePort(value) {
  ...
  if (!Number.isInteger(port) || port < 1 || port > 65535) {
    throw new import_commander.InvalidArgumentError("port must be an integer from 1 to 65535");
  }
  return port;
}
...
function parseDataChannels(value) {
  try {
    return validateDataChannels(Number(value));
  } catch (error) {
    throw new import_commander.InvalidArgumentError(formatError(error));
  }
}
main().catch((error) => {
  console.error(import_chalk4.default.red(formatError(error)));
  process.exitCode = 1;
});
