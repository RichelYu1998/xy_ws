import { buildPublicUrl, DEFAULT_DATA_CHANNELS, defaultTunnelLimits, isValidDataChannelCount, MAX_DATA_CHANNELS, PROTOCOL_VERSION, TUNNEL_KIND_EPHEMERAL, TUNNELS_API_PATH, } from "@hostc/protocol";
...
export async function handleRequest(request, env) {
    ...
    if (hostRoute.kind === "unknown") {
        return tunnelErrorResponse(request, {
            ...
            message: "This hostc tunnel does not exist, or the public URL is no longer valid.",
...
async function createTunnel(request, env, requestUrl) {
    ...
    const connectToken = await signToken(env.TOKEN_SECRET, createTokenPayload("connect", tunnelId, CONNECT_TOKEN_TTL_SECONDS, clientConnectionId));
    ...
    const response = {
        ...
        clientConnectionId,
        dataUrl: buildAbsoluteWebSocketUrl(requestUrl, `${TUNNELS_API_PATH}/${encodeURIComponent(tunnelId)}/channels`),
        connectToken,
        dataChannels,
...
async function connectChannel(request, env, tunnelId, route) {
    ...
    const internalUrl = new URL(`/_hostc/channels/${route.channelId}`, INTERNAL_ORIGIN);
    internalUrl.searchParams.set("clientConnectionId", payload.clientConnectionId);
    return env.HOSTC_TUNNEL.getByName(tunnelId).fetch(new Request(internalUrl, request));
}
async function initializeTunnelClientConnection(env, tunnelId, body) {
    const response = await env.HOSTC_TUNNEL.getByName(tunnelId).fetch(new Request(new URL("/_hostc/init", INTERNAL_ORIGIN), {
        method: "POST",
...
async function parseCreateTunnelDataChannels(request) {
...
function buildAbsoluteWebSocketUrl(requestUrl, pathname) {
...
function getEffectiveHostRoute(request, url, env) {
